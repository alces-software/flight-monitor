...
if [ ! -x /usr/bin/sinfo ] ; then
   echo "FAIL - No sinfo command found - is slurm installed?"
   exit 3
fi
...
if [ $unexpected -gt 0 ] ; then
   echo "FAIL - $unexpected node(s) reporting unexpected reboot ($down node(s) down)"
   exit 1
fi

if [ $killfailed -gt 0 ] ; then
   echo "FAIL - $killfailed node(s) reporting kill task failed"
   exit 1
fi

if [ $down -gt 0 ] ; then
   echo "FAIL - $down node(s) reporting down"
   exit 1
fi
...
if [ $drain -gt 0 ] ; then
   if [ $alcesdrain -eq $drain ] ; then
      ...
      echo "FAIL - $drain node(s) reporting drained"
      exit 1
   fi
fi
