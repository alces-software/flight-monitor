...
if [ ${slurmctld_flag} -ne 0 ] && [ ${slurmctld_flag} -ne 1 ]; then
    echo "FAIL - slurmctld flag is invalid. value is: ${slurmctld_flag}"
    exit 3
fi
...
if [ ${slurmdbd_flag} -ne 0 ] && [ ${slurmdbd_flag} -ne 1 ]; then
    echo "FAIL - slurmdbd flag is invalid. value is: ${slurmdbd_flag}"
    exit 3
fi
...
if [ ${slurmctld_active} -eq 0 ] && [ ${slurmdbd_active} -eq 0 ]; then
	...
	# slurmdbd is supposed to be on
        echo "FAIL - ${return_string_slurmctld} ${return_string_slurmdbd}"
        exit 1
    fi
