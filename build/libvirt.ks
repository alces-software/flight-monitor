...
lang en_GB
skipx
network --device eth0 --hostname monitor
...
part /var --fstype ext4 --size=16384 --ondisk=vda
...
curl http://10.151.0.10/repo/etc/scripts/fcm-setup.sh > /root/fcm-setup.sh
chmod +x /root/fcm-setup.sh
