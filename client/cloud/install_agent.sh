...
wget http://cfcgateway/resources/zabbix/zabbix_agent.tgz -O zabbix_agent.tgz
...
#Download custom scripts from cfcgateway
cd /opt/zabbix/scripts ; wget -r -nH --cut-dirs=3 --no-parent --reject="index.html*" http://cfcgateway/resources/zabbix/custom_zabbix_checks/
cd
...
wget http://cfcgateway/resources/zabbix/zabbix_agentd.conf -O /opt/zabbix/conf/zabbix_agentd.conf
